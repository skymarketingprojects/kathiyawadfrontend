...
const ProductFilter = () => {
    const { products, loading, bannerImage } = useProductFilterPage()
    return (
            ...
            <div className={` ${styles.container}`}>
                ...
                <div className={styles.productDisplay}>
                    ...
                    <div className={styles.productGrid}>
                        {loading ? <div>loading</div> : products.slice(0, 3).map((product, index) => (<ProductCard key={index} product={product} />))}
                    </div>
                </div>
            </div>
            <div className={styles.productGrid}>
                {loading ? <div>loading</div> : products.slice(3,).map((product, index) => (<ProductCard key={index} product={product} />))}
            </div>
        </div>
    )
}
